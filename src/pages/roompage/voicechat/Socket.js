...
// (20) 사용자가 방에 입장하는 함수
async function joinRoom(userId, roomNum) {
...
// (24) React 컴포넌트 - VoiceChatRoom
function VoiceChatRoom() {
    const [users, setUsers] = useState([]);
    const roomNum = 'roomNum1';

    // (25) 컴포넌트가 마운트될 때 더미 사용자들을 방에 추가
    useEffect(() => {
        ...
        dummyUsers.forEach((user, index) => {
            setTimeout(() => {
                joinRoom(user, roomNum);
            }, index * 1000); // 각 사용자 추가 시점을 1초 간격으로 설정
        });

        // (26) 컴포넌트 언마운트 시 사용자들을 방에서 나가게 처리
        return () => {
    ...
    return (
        <div className="room">
            <h2>{roomNum}</h2> {/* (27) 방 이름 표시 */}
            <div className="users">
                {users.map(user => (
                    <div key={user} className="user" id={`user-${user}`}>
                        <div className="user-avatar"></div> {/* 사용자 아바타 - 원형 구체 */}
                        <p>{user}</p> {/* 사용자 이름 표시 */}
                    </div>
                ))}
            </div>
